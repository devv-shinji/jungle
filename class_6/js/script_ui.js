    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            for(var i=0; i<loadData.length; i++) {
                ...
                var price = loadData[i].price;
                var reserve = loadData[i].reserve;

                $('.section.reservation > ol').append('\
                    <li class="unit">\
                        <button data-price='+price+' '+reserve+'>'+name+'</button>\
                    </li>\
                ');
            }

            // 동적으로 셋팅된 버튼에 클릭 이벤트
            $('.section.reservation > ol > li.unit > button').click(function() {
                // select클래스 토글기능(있으면 빼고, 없으면 넣어준다)
                ...
                $('.section.reservation > ol > li.unit > button').each(function(index) {
                    // 해당 순번의 버튼에 select라는 클래스가 있을 경우 true 발생(hasClass를 통해)
                    if($(this).hasClass('select')) {
                        // select를 갖고 있는 버튼 순서에서 작동
                        nameArr.push(loadData[index].name) // 배열에 선택된 자리명을 추가해줌

                        totalPrice += Number(loadData[index].price) // 총합계 변수에 선택된 자리가격을 더함
                    }
                })                
